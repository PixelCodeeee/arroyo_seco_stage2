// catalog-service/controllers/oferenteController.js
...
exports.obtenerOferentes = async (req, res) => {
    try {
        ...
        console.log('Parámetros recibidos:', { estado, tipo });
        ...
        if (estado || tipo) {
            ...
            data = await Oferente.findAllWithFilters(filters);
        } else {
            console.log('Sin filtros');
            data = await Oferente.findAll();
        }

        console.log(`Encontrados ${data.length} oferentes`);
        res.json({ total: data.length, oferentes: data });

    } catch (error) {
        console.error('ERROR DETALLADO:', error); // ← Esto mostrará el error real
        console.error('Stack trace:', error.stack); // ← Y la línea exacta
        res.status(500).json({ 
